...
export class LoginComponent implements OnInit {
  ...
  // HINT: Before SQL Injection fix (acc: ' OR 1=1 -- , pass: 123)
  // login() {
  //   this.user = {}
  ...
  login() {
    this.user = {};
    this.user.email = this._sanitizeEmail(this.emailControl.value);
    this.user.password = this._sanitizePassword(this.passwordControl.value);
    if (!this.user.email) {
      this.error = 'Invalid email input';
      return;
    }
    if (!this.user.password) {
      this.error = 'Invalid password input';
      return;
    }
    this.userService.login(this.user).subscribe((authentication: any) => {
      localStorage.setItem('token', authentication.token);
  ...
  _sanitizeEmail(email: string): string | null {
  ...
  _sanitizePassword(password: string): string | null {
