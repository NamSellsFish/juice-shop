...
export class PurchaseBasketComponent implements OnInit {
  ...
  //   }, (err) => { console.log(err) })
  // }
  // Front-end quản lý resource theo 2 hướng: 1. Local/Session Storage 2. Cần gì thì lấy từ network → Không dùng thư viện state management nào cả =((
  // After View Basket error fix
  load() {
    let payload = {} as any
    const token = localStorage.getItem('token')
    if (token) { payload = jwtDecode(token) }
    this.basketService.find(parseInt(sessionStorage.getItem('bid'), 10)).subscribe((basket) => {
      if (basket.UserId !== payload.data.id) { alert('Bad Hacker(s)!! You are not authorized to view this basket.'); return }
      if (this.isDeluxe()) {
        basket.Products.forEach(product => {
          product.price = product.deluxePrice
        })
